...
**Juice Mobile is a DePIN protocol on Solana that pays users in tokens for verified charging sessions — no hardware required, every proof on-chain, built for Latin America.**
...
Juice Mobile flips the model: **you generate the proof, you own the token, you keep the value.**
...
6. User withdraws via PIX or redeems JUICE tokens at TGE
...
**The token will be ownable.** JUICE will be an SPL token. Users will hold it, trade it, send it to any wallet. Loyalty points die with the app. JUICE lives on-chain — and every proof backing it is already there.
...
- **20 daily aggregate proofs on Solana mainnet** — each a Memo with SHA-256 of all sessions, immutable and auditable
...
- **Public dashboard** at `usejuicemobile.com` — live data, every proof linked to Solscan
...
The JUICE SPL token launches post-TGE. Every design decision is already locked — the on-chain proofs today are the pre-issuance ledger.

- **Supply:** 21,000,000,000 JUICE — fixed supply, mint authority renounced at launch
- **Emission:** 650 JUICE/hour of verified charging, halving every 4 years (Bitcoin-inspired schedule)
- **Conversion:** 1 point = 1 JUICE — every point already has an on-chain proof; conversion is 1:1 at TGE
- **Burn:** 2% of every token redemption permanently removed from circulation
- **Minimum claim:** 100,000 JUICE — prevents micro-transaction spam, ensures earned value is meaningful
- **Sustainability:** At 10,000 users averaging 30 min/day, the protocol runs for 45+ years with the halving schedule applied

Full tokenomics: `usejuicemobile.com/tokenomics`
...
- At TGE: JUICE token redemption transfers SPL tokens from project treasury to user's wallet (1:1 point conversion, already provable from existing Memos)
...
**Month 1 — Token Launch:** JUICE SPL token goes live. Points convert 1:1 to JUICE — the ledger is already on Solana, the conversion is mechanical. First ownable, tradeable asset backed by verified human attention.
...
> **Note:** Numbers updated 2026-05-11. Verify against Firebase Analytics before final submission.
